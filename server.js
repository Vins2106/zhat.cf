...
app.patch("/api/send/message", async (req, res) => {
  ...
  if (!findBot)
    return res.status(404).json({ error: { msg: "Bot not found" } });
...
app.patch("/api/post/message", async (req, res) => {
  ...
  let alr =
    (await messages.findOne({ ID: `${message.to}${message.author.UID}` })) ||
    (await messages.findOne({ ID: `${message.author.UID}${message.to}` }));
  if (!alr) {
    ...
    Promise.all(done)
      .then(() => {
        ...
        Promise.all(done2)
          .then(() => {
            console.log(
              `[MESSAGE_CREATE_EVENT] author: ${message.author.Username} to: ${message.to} content: ${message.content} time: ${message.time}`
            );
